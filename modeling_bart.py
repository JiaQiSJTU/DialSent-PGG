...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        # if config.constractive:
        #     self.weight = nn.Linear(outputs[0].size(-1), outputs[0].size(-1), bias=False)
    ...
    def forward(
        ...
        lm_logits = self.lm_head(outputs[0]) + self.final_logits_bias  # lm_logits:torch.Size([1, 59, 50265])

        if use_prefix:
            if exact_prefix == None:
                if data_name == "samsum":
                    pre_len = random.randint(1, 3) + 1  # samsum
                    # pre_len = 3 # samsum
                elif data_name == "dialsumm":
                    pre_len = random.randint(2, 4) + 1  # dialsumm
                    # pre_len = 4 # dialsumm
                lm_logits = lm_logits[:, pre_len:, :].contiguous()
                labels = labels[:, pre_len:].contiguous()
            else:

                for i in range(labels.size(0)):  # for each sample in batch
                    labels[i, :exact_prefix[i] + 1] = -100
